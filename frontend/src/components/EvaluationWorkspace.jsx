...
const emptyQuery = () => ({
  query: "", query_class: "factual", answerable: true, expected_speakers: [], date_start: "", date_end: "",
  judgments: [], acceptable_evidence_sets: [], reference_claims: [], hard_negative_ids: [], provenance: "human",
  human_reviewed: true, second_reviewer: "", adjudication_note: "",
});
...
export function EvaluationWorkspace({ controller }) {
  ...
  const importDataset = async (event) => {
    ...
    try { setDataset(JSON.parse(await file.text())); setSelected(0); setStale(null); } catch (reason) { setError(`Import failed: ${reason}`); }
    event.target.value = "";
  };
  ...
  return (
    <section className="audit-workspace evaluation-workspace">
      ...
      <div className="audit-actions">
        ...
        <button type="button" onClick={() => fileRef.current?.click()}>Import JSON</button><input ref={fileRef} hidden type="file" accept=".json" onChange={importDataset} />
      ...
      <div className="audit-grid">
        <div className="audit-column"><section><h3>Datasets</h3><table className="mini-table"><tbody>{datasets.map((item) => <tr key={item.path} onClick={safe(() => loadDataset(item.dataset_id))}><td>{item.name || "Invalid"}</td><td>{item.queries ?? "n/a"}</td><td>{item.corpus_fingerprint || item.error}</td></tr>)}</tbody></table></section>
          <section><h3>Dataset Identity</h3><label>Name<input value={dataset.name} onChange={(e) => setDataset({ ...dataset, name: e.target.value })} /></label><label>Corpus fingerprint<input value={dataset.corpus_fingerprint} onChange={(e) => setDataset({ ...dataset, corpus_fingerprint: e.target.value })} /></label></section>
        ...
        <div className="audit-column"><section><h3>Judgment Editor</h3>
          ...
          <label><input type="checkbox" checked={query.answerable} onChange={(e) => updateQuery({ answerable: e.target.checked })} /> Answerable</label>
          ...
          <label>Second reviewer<input value={query.second_reviewer || ""} onChange={(e) => updateQuery({ second_reviewer: e.target.value })} /></label><label>Adjudication note<textarea value={query.adjudication_note || ""} onChange={(e) => updateQuery({ adjudication_note: e.target.value })} /></label>
